...
Building on our baseline benchmarking results (see `cross-platform-benchmarking.md`), this paper investigates runtime optimization strategies for improving LLM inference throughput on the Intel Xe3-LPG integrated GPU (Panther Lake H, 25W, DDR5-5600). We test three optimizations independently on Llama 3.1 8B Instruct, then benchmark Qwen2.5-7B-Instruct as an alternative model to evaluate the impact of architecture and model size on bandwidth-constrained hardware.
