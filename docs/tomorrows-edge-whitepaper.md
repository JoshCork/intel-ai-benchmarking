...
Initial PTL benchmarks delivered **5.1 TPS using FP16 precision** — barely half the 10 TPS target (see [Cross-Platform Benchmarking Whitepaper](whitepaper.md), Section 7). This gap prompted stopgap proposals such as adding discrete Intel Arc GPUs. However, our working hypothesis was that this gap would close naturally through software and algorithmic improvements alone.

We were wrong about one thing: the gap didn't just close — it was obliterated. Through a systematic series of optimizations, each building on the last, we pushed PTL from 5.1 to **18.7 TPS** — exceeding the target by 87%:
...
- **[Cross-Platform Benchmarking](whitepaper.md)** — 5 platforms, 2 models, 3 precisions, full results
- **[Runtime Optimization Experiments](optimization-whitepaper.md)** — ov_config, GPTQ, GenAI pipeline analysis
- **[Semantic Quality Comparison](quality-comparison-whitepaper.md)** — 7 scenarios, 2 models, 3 precisions, response-level analysis
...
>
> **(b)** Quantization techniques are becoming near-lossless — our own testing confirms INT4 is indistinguishable from FP16 for kiosk conversations across all seven test scenarios (see [Quality Comparison](quality-comparison-whitepaper.md)).
>
...
- The **0.82 elasticity coefficient** was identical for both INT4 and INT8
...
The most powerful illustration of the compounding effect is the step-by-step progression from worst to best configuration on a single platform (Table 1, reproduced here for emphasis):

```
PTL iGPU, DDR5-5600:

  5.1 TPS ──[+165% quantization]──▶ 13.5 TPS ──[+9% runtime]──▶ 14.7 TPS
                                                                      │
  18.7 TPS ◀──[+6% model]── 17.6 TPS ◀──[+20% quant algorithm]──────┘
```
...
PTL is not "barely enough" — it is **production-ready with substantial margin.** A 50-token response takes 2.7 seconds with 65ms perceived-instant first token. For a retail kiosk or banking ATM, this is excellent user experience.
...
Full methodology details are available in [Cross-Platform Benchmarking](whitepaper.md), Section 6.
...
Cork, J. "Cross-Platform LLM Benchmarking on Intel Edge Hardware." February 2026. [`whitepaper.md`](whitepaper.md)

Cork, J. "Runtime Optimization Experiments for Edge LLM Inference." February 2026. [`optimization-whitepaper.md`](optimization-whitepaper.md)

Cork, J. "Semantic Quality Comparison: Quantization and Model Selection for Kiosk Deployment." February 2026. [`quality-comparison-whitepaper.md`](quality-comparison-whitepaper.md)
...
|----------|------------|-------|---------|-----|-----------|--------|
| Arc A770M (dGPU) | INT4 AWQ | Qwen 2.5-7B | GenAI | **52.2** | **42** | [Optimization](optimization-whitepaper.md) §6.2 |
...
|-------------|-----------|-----------|----------------|--------|
| FP16 → INT4 (AWQ) | +165% | +103% | None detected | [Benchmarking](whitepaper.md) §8 |
| AWQ → GPTQ | +21% | +47% | None/positive | [Optimization](optimization-whitepaper.md) §4 |
| optimum → GenAI C++ | +9% | +16% | None | [Optimization](optimization-whitepaper.md) §5 |
| Llama → Qwen | +27% | +45% | Slight improvement | [Quality](quality-comparison-whitepaper.md) §5 |
| ov_config flags | -0.7% to -7.4% | — | N/A | [Optimization](optimization-whitepaper.md) §3 |
| DDR5-5600 → DDR5-7200 | +18% | N/A | None | [Benchmarking](whitepaper.md) §11.9 |
...
|-------|------|-------|-------------|
| Cross-Platform Benchmarking | [`whitepaper.md`](whitepaper.md) | 5 platforms, 2 models, 3 precisions, full methodology | Best configs, bandwidth analysis, architecture comparison |
| Runtime Optimization | [`optimization-whitepaper.md`](optimization-whitepaper.md) | ov_config, GPTQ, GenAI pipeline on iGPU + dGPU | GenAI +9-16%, GPTQ +21-47%, gains fully additive |
| Quality Comparison | [`quality-comparison-whitepaper.md`](quality-comparison-whitepaper.md) | 7 scenarios, 2 models, 3 precisions, semantic analysis | INT4 = FP16 for kiosk; both models hallucinate; RAG essential |
